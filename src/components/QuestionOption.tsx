...
const QuestionOption: React.FC<QuestionOptionProps> = React.memo(({ text, isSelected, onClick }) => {
  return (
    ...
    >
      <span className={`text-sm sm:text-base ${isSelected ? 'text-[#0093B8]' : 'text-[#1C1A31]'} group-hover:text-[#0093B8]`}>
        {text}
      </span>
      <ChevronRight 
        className={`w-4 h-4 sm:w-5 sm:h-5 ${isSelected ? 'text-[#0093B8]' : 'text-gray-400'} 
